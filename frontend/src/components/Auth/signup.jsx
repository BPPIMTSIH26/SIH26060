...
import { Mail, Lock, User, Fingerprint, ArrowRight, Loader2, Building, Package, Shield, Upload, Eye, EyeOff, MapPin } from "lucide-react";
...
export default function Signup({ onSwitchMode }) {
    ...
    // Initialize the toast and navigation functions
    ...
    const [formData, setFormData] = useState({
        ...
        password: "",
        role: "station_master",
        station: "Maitri" // <-- ADDED: Default station for the toggle
    });
    ...
    const handleSignup = async (e) => {
        ...
        // Only attach the station if they are a station master
        ...
        try {
            ...
            localStorage.setItem("polar_twin_user", JSON.stringify({
                ...
                role: formData.role,
                station: formData.role === "station_master" ? formData.station : null, // Save station to session!
                avatar: "" 
            }));
            ...
            console.error(error);
            showToast(error.message || "Failed to register ID. Please try again.", "error");
        } finally {
            setLoading(false);
        }
    };
            ...
            <form onSubmit={handleSignup} className="space-y-4">
                        ...
                        <div className="flex flex-col gap-2.5 mb-6">
                            ...
                            <label className={`cursor-pointer flex items-center p-3 rounded-xl border transition-all duration-200 ${formData.role === 'station_master' ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400 shadow-sm' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                                ...
                                <div className="flex-1">
                                    <span className="block text-sm font-semibold">Station Master</span>
                                </div>
                            </label>
                            ...
                            <label className={`cursor-pointer flex items-center p-3 rounded-xl border transition-all duration-200 ${formData.role === 'logistics' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 shadow-sm' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                                ...
                                <div className="flex-1">
                                    <span className="block text-sm font-semibold">Logistics Officer</span>
                                </div>
                            </label>
                            ...
                            <label className={`cursor-pointer flex items-center p-3 rounded-xl border transition-all duration-200 ${formData.role === 'authority' ? 'border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 shadow-sm' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                                ...
                                <div className="flex-1">
                                    <span className="block text-sm font-semibold">High Authority</span>
                                </div>
                            </label>
                        </div>

                        <button type="button" onClick={() => setStep(2)} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold bg-cyan-600 hover:bg-cyan-700 text-white text-sm transition-all shadow-sm active:scale-[0.98]">
                ...
                {step === 2 && (
                    <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                        ...
                        {/* ROW 3: Station & Avatar (Grid adjusts automatically based on role) */}
                            ...
                            {formData.role === "station_master" && (
                                <div className="animate-in fade-in zoom-in-95 duration-300">
                                    ...
                                    <div className="flex gap-2 sm:gap-3 h-[40px] sm:h-[42px]">
                                        <label className={`flex-1 cursor-pointer flex items-center justify-center rounded-xl border transition-all duration-200 ${formData.station === 'Maitri' ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                                        ...
                                        <label className={`flex-1 cursor-pointer flex items-center justify-center rounded-xl border transition-all duration-200 ${formData.station === 'Bharati' ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                        ...
                        <div className="flex gap-3 pt-2">
                            ...
                            <button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold bg-cyan-600 hover:bg-cyan-700 text-white shadow-sm active:scale-[0.98] transition-all disabled:opacity-70 text-sm">
                                {loading ? <Loader2 className="animate-spin w-4 h-4" /> : <>Register ID <ArrowRight size={16} /></>}
                            </button>
                        </div>
                    </div>
                )}
